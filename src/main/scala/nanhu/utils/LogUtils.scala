...
import nanhu.utils.XSLogLevel.XSLogLevel
...
object XSLog {
  ...
  def apply(debugLevel: XSLogLevel)
           (prefix: Boolean, cond: Bool, pable: Printable)(implicit p: Parameter): Any =
  {
  ...
  def displayLog(implicit p: Parameter): Bool = {
    val debugOpts = p(DebugOptionsKey)
...
sealed abstract class LogHelper(val logLevel: XSLogLevel){
  ...
  def apply(cond: Bool, fmt: String, data: Bits*)(implicit p: Parameter): Any =
    apply(cond, Printable.pack(fmt, data:_*))
  def apply(cond: Bool, pable: Printable)(implicit p: Parameter): Any = apply(true, cond, pable)
  def apply(fmt: String, data: Bits*)(implicit p: Parameter): Any =
    apply(Printable.pack(fmt, data:_*))
  def apply(pable: Printable)(implicit p: Parameter): Any = apply(true.B, pable)
  def apply(prefix: Boolean, cond: Bool, fmt: String, data: Bits*)(implicit p: Parameter): Any =
    apply(prefix, cond, Printable.pack(fmt, data:_*))
  def apply(prefix: Boolean, cond: Bool, pable: Printable)(implicit p: Parameter): Any =
    XSLog(logLevel)(prefix, cond, pable)

  // trigger log or not
  // used when user what to fine-control their printf output
  def trigger(implicit p: Parameter): Bool = {
    XSLog.displayLog
  }

  def printPrefix()(implicit p: Parameter): Unit = {
  ...
  def exec(dump: () => Unit)(implicit p: Parameter): Unit = {
    when (trigger) {
      printPrefix
      dump
    }
  ...
  def exec(cond: Bool, dump: () => Unit)(implicit p: Parameter): Unit = {
    when (trigger && cond) {
      printPrefix
      dump
    }
